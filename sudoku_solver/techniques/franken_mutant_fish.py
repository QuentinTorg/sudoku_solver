"""Franken/Mutant Fish technique (restricted implementation).

Meaning:
    Extends fish logic by allowing box-based base units alongside row/column
    bases. This can produce eliminations that standard fish misses.
...
def apply_franken_mutant_fish(grid: Grid, candidates: dict[int, set[int]]) -> Step | None:
    """Apply a restricted franken/mutant fish elimination, else return None."""
    if not candidates:
        return None
    ...
    for digit in range(1, 10):
        ...
        return Step(
            ...
            rationale=(
                f"Digit {digit} forms a restricted franken/mutant fish "
